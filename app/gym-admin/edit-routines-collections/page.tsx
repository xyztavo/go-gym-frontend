      ...
      <div className="flex flex-col items-center justify-center p-2 m-2 gap-2 ">
        ...
        <div className="flex flex-row items-center justify-center flex-wrap p-2 border border-muted rounded-md gap-2">
          {isLoading ? (
            ...
            <>
              {data == null && (
                <ErrorDiv error="No routines found" statusCode={404} />
              )}
              {data &&
                data.map((routine) => {
                  return (
                    <Link
                      href={`/gym-admin/edit-routines-collections/${routine.id}`}
                      className="flex bg-background hover:scale-110 transition-transform flex-col items-center justify-center border border-muted rounded-md p-2 gap-2"
                      key={routine.id}
                    >
                      ...
                      />
                      <h1 className="text-xl font-semibold w-44 h-8 overflow-auto">{routine.name}</h1>
                      <p className="text-sm font-thin w-44 h-8 overflow-auto">{routine.description}</p>
                    </Link>
                  );
                })}
            </>
